...
describe Y2Storage::Proposal::AutoinstDrivesMap do
  ...
  let(:partitioning) { Y2Storage::AutoinstProfile::PartitioningSection.new_from_hashes(partitioning_array) }
  ...
  describe "#each" do
    it "executes the given block for each name/drive in the map" do
      ...
      end.to yield_successive_args(["/dev/sda", partitioning.drives[0]], ["/dev/sdb", partitioning.drives[1]])
    end
