...
describe Y2Storage::Filesystems::BlkFilesystem do
  ...
  describe "#preferred_mount_by" do
    ...
    # Sometimes we enforce the UUID, but in general new filesystems will only
    # have an UUID assigned after being created in the commit phase
    context "when the UUID is still not known" do
      let(:dev_name) { "/dev/sda2" }

      let(:by_uuid) { Y2Storage::Filesystems::MountByType::UUID }

      it "considers UUID as a valid option" do
        expect(Y2Storage::Filesystems::MountByType).to receive(:best_for) do |_dev, all_suitable|
          expect(all_suitable).to include by_uuid
          by_uuid
        end
      ...
      it "does not consider UUID as a valid option if :only_known is enforced" do
        expect(Y2Storage::Filesystems::MountByType).to receive(:best_for) do |_dev, all_suitable|
          expect(all_suitable).to_not include by_uuid
          by_uuid
        end

        subject.preferred_mount_by(only_known: true)
      end
    end
  ...
  describe "#reliable_path" do
    ...
    context "when the filesystem is not mounted" do
      before { subject.remove_mount_point }

      let(:preferred_mount_by) { Y2Storage::Filesystems::MountByType::DEVICE }

      it "returns the path corresponding to the preferred known mount_by" do
        expect(subject).to receive(:preferred_mount_by).with(only_known: true)
          .and_return(preferred_mount_by)
        expect(subject.reliable_path).to eq dev_name
      end
    end

    context "when the filesystem is mounted (or marked to be mounted)" do
      before { subject.mount_point.mount_by = mount_by }

      RSpec.shared_examples "fallback to preferred mount_by" do
        before do
          allow(subject).to receive(:mount_point).and_return(mount_point)
        end
      ...
      context "when mounting by UUID" do
        ...
        context "if the uuid of the filesystem is known already" do
          ...
          it "returns the by-uuid udev path" do
            expect(subject.reliable_path).to eq "/dev/disk/by-uuid/111222333444"
          end
        end

        context "if the uuid is still not known" do
          include_examples "fallback to preferred mount_by"
        end
      end
      ...
      context "when mounting by label" do
        ...
        context "if the filesystem has a label" do
          it "returns the by-label udev path" do
            expect(subject.reliable_path).to eq "/dev/disk/by-label/root"
          end
        end
      ...
      context "when mounting by path" do
        ...
        context "if the block device has by-path udev paths" do
          ...
          it "returns the first by-path udev path" do
            expect(subject.reliable_path).to eq(paths.first)
          end
        end
      ...
      context "when mounting by id" do
        ...
        context "if the block device has by-id udev paths" do
          ...
          it "returns the first by-id udev path" do
            expect(subject.reliable_path).to eq(ids.first)
          end
        end
