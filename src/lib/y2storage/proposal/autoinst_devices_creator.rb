...
require "y2storage/proposal/partitioner"
  ...
  module Proposal
    ...
    private
      ...
      def process_partitions
        planned_partitions = sized_partitions(planned_devices.disk_partitions, devicegraph: devicegraph)
        parts_to_reuse, parts_to_create = planned_partitions.partition(&:reuse?)

        Partitioner.new(devicegraph).reuse_partitions(parts_to_reuse)
      ...
      def reuse_partitionables(reused_devices)
        reused_devices.each_with_object(creator_result) do |dev, result|
          partitioner = Partitioner.new(result.devicegraph)
      ...
      def create_partitions(new_partitions, devicegraph)
        ...
        partitioner = Partitioner.new(devicegraph)
